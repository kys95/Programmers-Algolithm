...
1. 검색 키워드와 words의 단어 길이를 기준으로 새로운 리스트를 생성한다.

2. 키워드의 '?'가 접두사에 올 경우 이진 탐색을 할 수 없으므로 문자열을 뒤집은 리스트를 생성한다.

3.  '?'가 키워드의 접두사, 접미사에 오는 경우에 따라 이진탐색을 수행하여 해당 키워드와 매치된 단어 갯수를 반환한다.
