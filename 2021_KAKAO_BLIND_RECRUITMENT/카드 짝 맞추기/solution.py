...
# 1. Permutation을 사용하여 선택할 카드의 순서를 정한다. -> 어떤 카드를 먼저 선택해야 조작 횟수가 최소가 될지는 탐색해보기 전에는 알 수가 없습니다.
#   그래서 저는 카드를 선택하는 모든 경우의 수를 구하고 완전 탐색을 진행한다. 완전 탐색을 해도 크게 부담이 되지 않는 이유는
#   먼저 board의 크기가 4x4이고 카드의 종류도 많지 않기 때문이다.
# 2. 모든 경우의 순서에 대해 조작 횟수가 가장 적은 것을 반환한다.